...
[Serializable]
public class FingerDrawUnityEvent : UnityEvent<Vector2>
{
    ...
    BaseFocusHandler, IMixedRealityTouchHandler, IMixedRealityPointerHandler, IMixedRealitySourceStateHandler
{
    ...
    [Header("Events")]
    public FingerDrawUnityEvent DrawStarted = new FingerDrawUnityEvent();
    public FingerDrawUnityEvent DrawStopped = new FingerDrawUnityEvent();
    public FingerDrawUnityEvent DrawUpdated = new FingerDrawUnityEvent();
    ...
    private Vector2 GetUv()
    {
        if (TouchActive && AreSourcesCompatible())
        {
            ...
            foreach (uint key in handDataMap.Keys)
            {
                offset += handDataMap[key].touchingQuadCoord;
            }
            offset /= (float)handDataMap.Count;
            return offset;
        }
